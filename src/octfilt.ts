 ...
 * @returns 
 */
export async function octfilt(data: Float32Array, sampleRate: number): Promise<Float32Array> {
  const offlineCtx = new OfflineAudioContext(1, data.length, sampleRate);
  const filter = offlineCtx.createIIRFilter(COEFFICIENTS[0].b, COEFFICIENTS[0].a);
