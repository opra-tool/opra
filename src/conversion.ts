...
import {
  filterChainFrequencyResponse,
  frequencyResponse,
} from './octave-band-filtering/frequency-response';
 ...
 */
export async function binauralToMonaural(buffer: IRBuffer): Promise<IRBuffer> {
  buffer.assertStereo();
  ...
  return new IRBuffer(meanSamples, buffer.sampleRate);
}
