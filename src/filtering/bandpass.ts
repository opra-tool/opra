...
export function bandpass(
  ...
  for (let i = 0; i < numberOfBiquads; i += 1) {
    ...
    const beta = sqrt(
      subtract(new Complex(1, 0), pow(multiply((BW / F0) * 0.5, pLp[i]), 2))
    );
    pa[i] = multiply(
      2 * Math.PI * F0,
    ...
    p[i] = divide(
      add(new Complex(1, 0), divide(pa[i], 2 * fs)),
      subtract(new Complex(1, 0), divide(pa[i], 2 * fs))
    );
  }
  ...
  for (let i = 0; i < numberOfBiquads; i += 1) {
    const aa = a[i];
    const h = frequencyResponse(b, aa, [f0, f0], fs);
